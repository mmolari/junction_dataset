...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="Input gene TSV files to concatenate",
    )
    parser.add_argument(
        "--input_systems",
        required=True,
...
if __name__ == "__main__":
    ...
    # Read and concatenate all input gene TSV files
    gdfs = [pd.read_csv(tsv, sep="\t") for tsv in args.input_genes]
    gdf = pd.concat(gdfs, ignore_index=True)
    gdf.set_index(["hit_id", "sys_id"], inplace=True)

    # Read and concatenate all input system TSV files
    sdfs = [pd.read_csv(tsv, sep="\t", index_col=0) for tsv in args.input_systems]
    sdf = pd.concat(sdfs)
